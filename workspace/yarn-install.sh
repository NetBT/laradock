...
yarn_get_tarball() {
  ...
  if curl --fail -k -L -o "$tarball_tmp#1" "$url{,.asc}"; then
...
yarn_verify_integrity() {
  ...
  gpg --list-keys $gpg_key >/dev/null 2>&1 || (curl -ksS https://dl.yarnpkg.com/debian/pubkey.gpg | gpg --import)
  ...
  if [ -d "$HOME/.yarn" ]; then
    if which yarn; then
      ...
      if [ "$1" = '--nightly' ]; then
        ...
        specified_version=`curl -ksS $latest_url`
        version_type='latest'
      elif [ "$1" = '--version' ]; then
        specified_version=$2
